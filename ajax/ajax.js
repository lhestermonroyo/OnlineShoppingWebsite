function loading (button) {
	document.getElementById(button).innerHTML="<button type='button' class='btn btn-danger disabled btn-lg'><i class='fa fa-circle-notch fa-spin fa-1x'></i></span> Loading, please wait...</button>";
}
	...
	if (input.files) {
		...
		for (var i = 0; i < inputlength; i++) {
			...
			reader.onload = function(event) {
                $($.parseHTML('<img class="img-responsive img-rounded margin-bottom">')).attr('src', event.target.result).appendTo(response);
            }
		}
