...
<html>
<head>
	<title>Messages &bull; Guimaras Product and Souvenir Online Retail</title>
	...
	<div class="row">
		...
		?>	
			<div class="input-group search-extended">
				...
				<input type="text" class="form-control input-lg" name="search" placeholder="Search from over <?=$cnt_seller_row['seller'];?> conversations..." onkeyup="load_process ('convo-container', '../functions/search-module.php?seller_id=<?=$seller_row['acc_id'];?>&search-convo-2='+this.value)" autocomplete="off">
			</div>
			<hr>
				...
				if (!empty($_GET['cos_id'])) {
					...
					<div class="panel-group">
						<div class="panel panel-default">
							...
							<div class="panel-body" style="height: 375px; overflow-y: auto;">
							<div class="panel btn-default">
								<?php
									if (empty($mes_row['convo_started'])) {
									?>
									<div class="panel-body">
										<p class="text-center margin-top">Compose your message below to send message.</p>
									</div>
									<?php	
									}
									else {
									?>
									<div class="panel-body">
										<p class="text-center margin-top">Your conversation started last <?=date("F j, Y - h:ia", strtotime($mes_row['convo_started']));?></p>
									</div>
									<?php	
									}
								?>
							...
							<hr>
							<span id="convo-response<?=$mes_row['mes_id'];?>">
