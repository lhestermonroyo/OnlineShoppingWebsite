<?php
	...
	if (isset($_GET['item_id'])) {
		...
		$delete_query5 = $conn->query("delete from wishlists_tbl where item_id = '".$_GET['item_id']."'");
	...
	if (isset($_GET['wish_id'])) {
		$wish_query = $conn->query("select * from wishlists_tbl where wish_id = '".$_GET['wish_id']."'");
		$wish_row = $wish_query->fetch();
		$get_item_query = $conn->query("select * from items_tbl where item_id = '".$wish_row['item_id']."'");
		$get_item_row = $get_item_query->fetch();

		$delete_query = $conn->query("delete from wishlists_tbl where wish_id = '".$_GET['wish_id']."'");
		?>
		<div class="alert alert-success padding-lg">
			<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
			<strong><i class="fa fa-check fa-fw"></i></strong> Item '<?=$get_item_row['name'];?>' has been removed to your wishlist.</a>
		</div>
		<?php
	}
