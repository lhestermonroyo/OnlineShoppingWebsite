<?php
	...
	if (isset($_POST)) {
		$chk_account_query = $conn->query("select count(acc_id) as acc_id from accounts_tbl where email = '".$_POST['email']."' and acc_type = 'Admin'");
		...
		if ($chk_account_row['acc_id'] == 0) {
			...
			if ($save_query->execute()) {
				...
				$save_query2 = $conn->prepare("insert into admins_tbl (acc_id, firstname, lastname)values(:acc_id, :firstname, :lastname)");
				...
				<script type="text/javascript">
					window.location.assign("../admin/admin-accounts.php");
				</script>
				<?php
				}
				else {
