	...
	if (isset($_POST['email']) && isset($_POST['password']) && isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['birthdate']) && isset($_POST['gender']) && isset($_POST['address'])) {
		...
		if ($chk_email_row['acc_id'] == 0) {
			if ($chk_name_row['acc_id'] == 0) {
				$save_query = $conn->prepare("insert into accounts_tbl (email, password, acc_type)values(:email, :password, :acc_type)");
				...
				if ($save_query->execute()) {
					...
					?>
						<div class="alert alert-success padding-lg">
						  <strong><i class="fa fa-check fa-fw"></i></strong> Sign up successful. You can now log in using your email and password.
						</div>
					<?php
					}
					else {
					?>
						<div class="alert alert-danger">
						  ...
						  <strong><i class="fa fa-exclamation-circle fa-fw"></i></strong> Sign up failed. Please try again.
						</div>
						<form class="row" onsubmit="loading ('btn-response')" autocomplete="off" id="signup-form" method="POST">
				          ...
				          <div class="col-lg-12 col-sm-12">
				              ...
				              <button type="submit" class="btn btn-danger btn-lg" onclick="form_process ('signup-form', 'functions/verify-signup.php', 'signup-response')">Create Account</button>
				            </span>  
				          </div> 
				        </form>
					<?php
