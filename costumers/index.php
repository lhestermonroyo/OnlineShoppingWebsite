...
<html>
<head>
	<title>Home &bull; Guimaras Product and Souvenir Online Retail</title>
			...
			<div class="row">
				<div class="col-lg-10 col-sm-10">
					<div class="input-group search-extended">
						...
						<input type="text" class="form-control input-lg" name="search" placeholder="Search from over <?=$cnt_item_row['item_id'];?> items..." onkeyup="load_process ('search-response', '../functions/search-module.php?search-item='+this.value)" autocomplete="off">
					</div>
					<span id="search-response"></span>
				</div>
				<div class="col-lg-2 col-sm-2">
					<div class="dropdown">
					  <button class="btn btn-info btn-lg btn-block dropdown-toggle" type="button" data-toggle="dropdown">Sort by <span class="caret"></span></button>
		...
		<div class="row">
			<div class="col-lg-3 col-sm-3" id="myScrollspy">
			  <div data-spy="affix" data-offset-top="70">	
				...
				<a href="index.php" style="height: 45px;" class="btn btn-danger btn-lg btn-block"><span class="pull-left">View All</span><i class="fa fa-chevron-right fa-fw pull-right" style="margin-top: 2px;"></i></a>
				<?php
				$category_query = $conn->query("select * from categories_tbl");
				while ($category_row = $category_query->fetch()) {
				?>
				<a href="index.php?category=<?=$category_row['cat_id'];?>" style="height: 45px;" class="btn btn-info btn-lg btn-block"><span class="pull-left"><?=$category_row['category_name'];?></span><i class="fa fa-chevron-right fa-fw pull-right" style="margin-top: 2px;"></i></a>
				<?php
				}
				?>
				<?php
					...
					$cnt_wish_query = $conn->query("select count(wish_id) as wish_id from wishlists_tbl where acc_id = '".$cos_row['acc_id']."'");
					$cnt_wish_row = $cnt_wish_query->fetch();
				?>
				<hr>
				<h4 class="margin-bottom"><i class="fa fa-shopping-bag fa-fw"></i> Sellers and Stuff</h4>
				<button class="btn btn-danger btn-lg btn-block" data-toggle="modal" data-target="#modal-sellersearch"><span class="pull-left">Sellers</span><span class="badge pull-right" style="margin-top: 4px;"><?=$cnt_seller_row['acc_id'];?></span></button>
				<button class="btn btn-info btn-lg btn-block" data-toggle="modal" data-target="#modal-mycart"><span class="pull-left">My Cart</span><span class="badge pull-right" style="margin-top: 4px;"><?=$cnt_cart_row['cart_id'];?></span></button>	
				<button class="btn btn-info btn-lg btn-block" data-toggle="modal" data-target="#modal-mywishlist"><span class="pull-left">My Wishlist</span><span class="badge pull-right" style="margin-top: 4px;"><?=$cnt_wish_row['wish_id'];?></span></button>
			  </div> 	
			</div>
			<?php
				include "../modals/seller-search.php";
				include "../modals/my-cart.php";
				include "../modals/my-wishlist.php";
			?>
								  ...
								  <div class="panel panel-default panel-hover">
								  	...
								  	<div class="panel-body">
										...
										<p class="text-small text-muted text-center padding-bottom border-bottom">by: <?=$item_row['firstname'];?> <?=$item_row['lastname'];?></p>
								  		...
								  		<button class="btn btn-danger btn-lg btn-block" data-toggle="modal" data-target="#modal-addtocart<?=$item_row['item_id'];?>">Add to Cart <i class="fa fa-shopping-cart fa-fw"></i></button>
								  	</div>
								  </div>
								</div>  	
								</span>
