    ...
    <div class="container" style="margin-top: 50px; margin-bottom: 75px;">
		<div class="row margin-top" style="margin-bottom: -12px;">
    		<div class="col-lg-6 col-sm-6">
    			...
    			<a data-toggle="modal" data-target="#addcat" class="btn btn-info btn-lg margin-left pull-left" style="margin-top: 13px;"><i class="fa fa-plus fa-fw"></i> Add Category</a>
    			<a data-toggle="modal" data-target="#modal-unitsettings" class="btn btn-info btn-lg margin-left-sm pull-left" style="margin-top: 13px;"><i class="fa fa-archive fa-fw"></i> Unit Settings</a>
    		</div>	
    	</div>
		...
		<ul class="nav nav-pills nav-justified">
			...
			?>
		  	<li><a href="admin-inventory.php" class="btn-lg">All Items</a></li>
		  <?php
			...
			while ($category_row = $category_query->fetch()) {
				...
				?>
				 <li class="active"><a href="admin-category.php?cat_id=<?=$category_row['cat_id'];?>" class="btn-lg"><?=$category_row['category_name'];?></a></li>
				<?php	
				}
				else {
				?>
				 <li><a href="admin-category.php?cat_id=<?=$category_row['cat_id'];?>" class="btn-lg"><?=$category_row['category_name'];?></a></li>
				<?php
				}
			}
		  ?> 
    		...
    		<div class="col-lg-6 col-sm-6">	
				<div class="btn-group pull-right margin-top">
					<div class="input-group search-unextended pull-left">
					    ...
					    <input type="text" class="form-control input-lg" name="search" placeholder="Search <?=$cat_row['category_name'];?>..." onkeyup="load_process ('item-container', '../functions/search-module.php?cat_id=<?=$cat_row['cat_id'];?>&search-item-3='+this.value)" autocomplete="off">
					</div>
				</div>	
			</div>	
    	</div>
